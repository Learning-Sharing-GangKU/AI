...
class RoomRepository:
    ...
    """
    def list_rooms(self) -> List[RoomMeta]:
        now = datetime.now(timezone.utc)
        return [
            RoomMeta(room_id=1,  title="토익 800+ 주말 스터디", category="스터디", region="강남", member_count=12, updated_at=now),
            RoomMeta(room_id=2,  title="아침 러닝 크루",       category="운동",   region="서초", member_count=8,  updated_at=now),
            RoomMeta(room_id=3,  title="한 달 한 권 독서",    category="독서",   region="종로", member_count=20, updated_at=now),
            RoomMeta(room_id=4,  title="보드게임 모임",       category="게임",   region="마포", member_count=5,  updated_at=now),
            RoomMeta(room_id=5,  title="버스킹 보컬 연습",    category="음악",   region="홍대", member_count=7,  updated_at=now),
            RoomMeta(room_id=6,  title="주말 자원봉사",       category="봉사",   region="강동", member_count=3,  updated_at=now),
            RoomMeta(room_id=7,  title="국내 당일치기 여행",  category="여행",   region="수원", member_count=10, updated_at=now),
        ]
...
# -------------------------------------------------------
@router.post("/recommendations", response_model=RecommendationResponse, summary="사용자 선호 기반 방 추천")
async def recommend(req: RecommendByCategoryRequest) -> RecommendationResponse:
    """
    호출 시점:
    ...
    try:
        ...
        if len(req.preferred_categories) > 3:
            raise HTTPException(status_code=400, detail="preferred_categories는 최대 3개까지 허용됩니다.")
        ...
        recommender = Recommender(cat_index=CAT_INDEX)
        ...
        return RecommendationResponse(
            ...
            page=req.page,
            limit=req.limit,
            debug={"strategy_version": debug.get("strategy_version", "rec_v1"),
